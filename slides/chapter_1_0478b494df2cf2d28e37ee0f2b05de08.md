...
`@script`
To illustrate the effects of dataset shift, we split the data in two batches of 20000 examples sorted by date, and hold out the last few thousand examples to assess accuracy. Training the same model in the older batch produces lower accuracy against the latest data than training on the more recent batch. The literature suggests that a change in the structure of electricity provision in New South Wales, where this dataset is from, during the period in question caused the relationship between the features and the electricity price to change. In general, you might not be able to identify why your dataset has shifted, but it is always a possibility to keep in mind: the world tends to be fairly dynamic! The good news is that dataset shift does not always hurt your model: in the example on the bottom right corner, a movement of the two classes leaves the decision boundary unaffected, so that nothing needs to be done despite the shift.
